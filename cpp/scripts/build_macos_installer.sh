...
INSTALLER_APP="$DIST_DIR/Install Network Tools 1.0.8.app"
DMG_PATH="$DIST_DIR/Network-Tools-1.0.8-macos-installer.dmg"
TMP_DIR="$(mktemp -d)"
...
mkdir -p "$PAYLOAD_DIR"
cp -R "$PACKAGED_APP" "$PAYLOAD_DIR/Network Tools 1.0.8.app"
...
plist_write "CFBundleName" "Install Network Tools 1.0.8"
plist_write "CFBundleDisplayName" "Install Network Tools 1.0.8"
...
hdiutil create -volname "Network Tools 1.0.8 Installer" -srcfolder "$STAGE_DIR" -ov -format UDZO "$DMG_PATH" >/dev/null
